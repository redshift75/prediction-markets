...
const VenueRenderer = makeReactRenderer((p) => {
  ...
  const src = venue === "Polymarket" ? r.polymarketLogo || res.polymarketLogo || "assets/polymarket.png" : r.kalshiLogo || res.kalshiLogo || "assets/kalshi.png";
  return (
    <img src={src} alt={venue} title={venue} style={{ width: 16, height: 16, borderRadius: 4, display: 'block', objectFit: 'cover' }} />);
...
const TagsRenderer = makeReactRenderer((p) => {
  ...
  return <span className="truncate tag-list">{text}</span>;
});
...
function MarketsGrid({ rows, selectedId, onSelectRow, selected, setSelected, onSubscribe, isSubscribed, daysChange }) {
  ...
  React.useEffect(() => {
    ...
    { headerName: "Venue", field: "venue", width: 38, minWidth: 34, maxWidth: 48,
      headerClass: "ag-center-header ag-tight-header",
      cellClass: "ag-tight-cell",
    ...
    { headerName: "Event Contract", field: "event", flex: 2.1, minWidth: 300,
      cellRenderer: EventRenderer, tooltipField: "event" },
    { headerName: "Outcome", field: "contract", flex: 2.4, minWidth: 330,
    ...
    { headerName: "Category Tag", colId: "categoryTag",
      ...
      width: 78, minWidth: 70,
      flex: 0, suppressSizeToFit: true,
      cellRenderer: TagsRenderer,
      tooltipValueGetter: (p) => p.value },
    { headerName: "Curr Prob", field: "currProb", width: 64, minWidth: 60,
      headerClass: "ag-center-header ag-compact-header",
      cellClass: "ag-compact-cell",
      cellStyle: { display: "flex", alignItems: "center", justifyContent: "center" },
      cellRenderer: PercentRenderer },
    { headerName: `${daysChange || 1}d Prob Chg`, field: "probChg", colId: "probChg", width: 76, minWidth: 70,
      headerClass: "ag-center-header ag-compact-header",
      cellClass: "ag-compact-cell",
      cellStyle: { display: "flex", alignItems: "center", justifyContent: "center" },
      cellRenderer: TrendNoIconRenderer,
      comparator: (a, b) => Number(a) - Number(b) },
    { headerName: `${daysChange || 1}d Z-Score`, field: "zScore", colId: "zScore", width: 72, minWidth: 66,
      headerClass: "ag-center-header ag-compact-header",
      cellClass: "ag-compact-cell",
      cellStyle: { display: "flex", alignItems: "center", justifyContent: "center" },
      cellRenderer: ZRenderer,
      comparator: (a, b) => Number(a) - Number(b) },
    { headerName: "Prior Prob", field: "priorProb", colId: "priorProb", width: 70, minWidth: 64,
      headerClass: "ag-center-header ag-compact-header",
      cellClass: "ag-compact-cell",
      cellStyle: { display: "flex", alignItems: "center", justifyContent: "center" },
      cellRenderer: PercentRenderer },
    { headerName: "Total Volume", field: "volume", width: 88, minWidth: 80,
      headerClass: "ag-center-header ag-compact-header",
      cellClass: "ag-compact-cell",
      cellStyle: { display: "flex", alignItems: "center", justifyContent: "center" },
      cellRenderer: MoneyRenderer,
      comparator: (a, b) => Number(a) - Number(b) },
    { headerName: "# Trades", field: "numTrades", width: 66, minWidth: 60,
      headerClass: "ag-center-header ag-compact-header",
      cellClass: "ag-compact-cell",
      cellStyle: { display: "flex", alignItems: "center", justifyContent: "center" },
    ...
    { headerName: "Expiry Date", field: "expiry", width: 80, minWidth: 74,
      headerClass: "ag-center-header ag-compact-header",
      cellClass: "ag-compact-cell",
      cellStyle: { display: "flex", alignItems: "center", justifyContent: "center" },
      cellRenderer: MonoRenderer },
    { headerName: "Subscribe", field: "__subscribed", width: 66, minWidth: 60,
  ...
  React.useEffect(() => {
    ...
    if (allDefs) {
      const updated = allDefs.map((c) => {
        ...
        if (c.colId === "priorProb" || c.field === "priorProb") return { ...c, headerName: "Prior Prob" };
        return c;
      });
  ...
  return (
    <div className="expand-inner">
      <div className="card">
        ...
        <div className="chart-wrap" style={{ height: "30px" }}>
...
function BrowsePage({ subscriptions, addSubscription, watchlists, setWatchlists, focusListId, clearFocusList }) {
  ...
  const [activeList, setActiveList] = React.useState(focusListId || "biggest_movers"); // list id
  React.useEffect(() => {
  ...
  // Apply system-list filter presets when list changes (including initial load)
  const prevListRef = React.useRef(null);
  React.useEffect(() => {
  ...
  return (
    <>
      <div className="content" style={{ padding: "4px 24px 16px", height: "950px" }}>
        <div className="content-main">
          <div className="split-stacked">
            <div className="split-top">
              <div className="card" style={{ height: '100%', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                <div style={{ padding: '0 0 0 4px', fontSize: 12, color: 'hsl(var(--muted-foreground))', fontWeight: 500, letterSpacing: 0.2 }}>
                  Showing {filtered.length} of {MARKETS.length} Contracts
                </div>
                <div className="card-header" style={{ gap: 8, flexWrap: 'nowrap', display: 'flex', alignItems: 'center', overflow: 'hidden', padding: '4px 8px 4px 0', borderBottom: 'none' }}>
                  {(() => {
                    const pillOrder = [
                    ...
                    { id: "all_markets", name: "All Active Contracts" },
                    { id: "expired_contracts", name: "Historical Contracts" }];
                  ...
                  <div style={{ marginLeft: 'auto', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ position: 'relative' }}>
                      <input
                        className="input"
                        placeholder="Search Event Contracts & Outcomes..."
                        value={search}
                        onChange={(e) => setSearch(e.target.value)}
                        style={{ width: 260, paddingLeft: 30, background: 'hsl(var(--primary) / 0.08)', borderColor: 'hsl(var(--primary) / 0.35)', fontWeight: 500 }} />
